...
export const validateUserInput = (input: string): GuardrailResult => {
  ...
  if (isSystemPromptAttempt(input)) {
    return {
      ...
      suggestedResponse:
        "I'm here to help you learn about Eduard. What would you like to know about his projects or background?",
    }
  }
  ...
  if (isJailbreakAttempt(input)) {
    return {
      ...
      suggestedResponse:
        'I can only answer questions about Eduard. What would you like to know about his work or projects?',
    }
  }
  ...
  if (isOffTopic(input)) {
    return {
      ...
      suggestedResponse:
        "I'm sorry, I can only answer questions about Eduard. Feel free to ask about his projects, skills, or background!",
    }
  }
...
export const validateAIResponse = (response: string): GuardrailResult => {
  ...
  const properRedirect = /i'm sorry.*only.*eduard|can only answer.*eduard|about eduard/i.test(
    cleanResponse,
  )
